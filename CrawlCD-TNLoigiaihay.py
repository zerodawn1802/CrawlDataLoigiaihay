...
# Hàm xử lý nội dung đệ quy
def process_element(element, doc, title, image_dir, headers, paragraph=None, bold=False, italic=False, underline=False):
    # if not paragraph:  # Chỉ tạo paragraph mới nếu chưa có
    ...
    if isinstance(element, NavigableString):  # Văn bản thô
        ...
        for child in element.children:
            if isinstance(child, NavigableString):
                ...
                # Truyền định dạng hiện tại của thẻ cha vào thẻ con
    ...
    elif element.name:  # Các thẻ khác (strong, span, em, u, v.v.)
        # Truyền định dạng từ thẻ cha vào thẻ con, kể cả khi là <span>
...
def crawl_with_url(url, output_dir="articles"):
    ...
    if responses.status_code == 200:
        ...
        count = 1
        for i in box_contents.find_all('div', class_='wiki-header-search'):
            i.decompose()
        for i in box_contents.find_all('script'):
            i.decompose()
...
url = "https://loigiaihay.com/tong-hop-ca-dao-tuc-ngu-ve-cha-me-e36239.html"
crawl_with_url(url)