...
def crawl_single_page(url, output_dir="van12chantroisangtao"):          #Thay đổi tên thư mục output
    ...
    if response.status_code == 200:
        ...
        # Lấy tiêu đề
        title = soup.find('h1', class_='magb5').text.strip()
        ...
        for div in content_divs:
            ...
            for element in div.children:
                ...
                process_element(element, doc, title, image_dir, header, paragraph, bold=False, italic=False, underline=False)
...
def crawl_whole_page(url):
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    responses = requests.get(url, headers=headers)
    if responses.status_code == 200:
        soups = BeautifulSoup(responses.text, 'html.parser')
        
        url_divs = soups.find_all('div', class_='wrap-width50or100')
        for url_div in url_divs:
            urls = url_div.find_all('a')
            for url in urls:
                tmp_link = url.get('href')
                if not tmp_link.startswith('http'):
                    tmp_link = 'https://loigiaihay.com' + tmp_link
                crawl_single_page(tmp_link)
    
# URL của trang muốn crawl
url = "https://loigiaihay.com/tom-tat-bo-cuc-van-12-chan-troi-sang-tao-c1850.html"  # Thay bằng URL của bạn
crawl_whole_page(url)